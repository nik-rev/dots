        ...
        WalkDir::new(path)
            ...
            .try_for_each(|file| {
                ...
                let (file_contents, new_location) = if let Some(first_line) =
                    ...
                    && let Some(marker_start_pos) = first_line.find(MarkerArgs::MARKER)
                    && let Some(marker_args) =
                        first_line.get(marker_start_pos + MarkerArgs::MARKER.len()..)
...
impl Link {
    ...
    pub fn process(self, root: &Path) -> Result<()> {
        ...
        let marker = marker.as_ref().map_or(String::new(), |v| {
            format!("{}{v}", MarkerArgs::MARKER).pipe(|it| commented::comment(it, &path)) + "\n"
        });
        ...
        fs::write(&path, {
            let marker = contents
                .lines()
                .next()
                .filter(|line| line.contains(MarkerArgs::MARKER))
                .map(|line| format!("{line}\n"))
                .unwrap_or_default();
            let contents = if let Some(first_line) = contents.lines().next()
                && first_line.contains(MarkerArgs::MARKER)
            {
            ...
            let header = [
                format!("@generated by `{}`", MarkerArgs::MARKER),
